...
static inline c_SimpleXMLElement* element_from_object(CObjRef obj) {
  return obj.getTyped<c_SimpleXMLElement>();
}
...
static Object _node_as_zval(c_SimpleXMLElement* sxe, xmlNodePtr node, 
  ...
  c_SimpleXMLElement* subnode = element_from_object(obj);
...
static void php_sxe_move_forward_iterator(c_SimpleXMLElement* sxe) {
  ...
  if (!sxe->iter.data.isNull()) {
    c_SimpleXMLElement* intern = element_from_object(sxe->iter.data);
    node = intern->node;
...
static xmlNodePtr php_sxe_get_first_node(c_SimpleXMLElement* sxe, 
  ...
  if (sxe && sxe->iter.type != SXE_ITER_NONE) {
    ...
    if (!sxe->iter.data.isNull()) {
      retnode = element_from_object(sxe->iter.data)->node; 
    }
    return retnode;
  } else {
    return node;
  ...
  if (node) {
    ...
    if (elements) {
      ...
      if (member.isNull() || member.isInteger()) {
        ...
        } else if (/*type == BP_VAR_W || type == BP_VAR_RW*/ true) {
      ...
      while (attr) {
        if ((!test || !xmlStrcmp(attr->name, sxe->iter.name)) &&
          ...
          zattr.set(String((char*)attr->name),
                    sxe_xmlNodeListGetString((xmlDocPtr)sxe->document, 
                                             attr->children, 1));
        }
        attr = attr->next;
      }
...
static Variant sxe_get_value(CObjRef z) {
  return sxe_object_cast(element_from_object(z), HPHP::KindOfString);
}
...
static bool sxe_prop_dim_write(c_SimpleXMLElement* sxe, CVarRef member,
  ...
  if (member.isNull() || member.isInteger()) {
    ...
    if (!member.isString()) {
      // TODO: Make it a string, and trim it.
      /* php_trim(Z_STRVAL(trim_zv), Z_STRLEN(trim_zv), nullptr, 0,
       *          &tmp_zv, 3); */
    }
  ...
  if (value.isString() || value.isInteger() || value.isBoolean() ||
      value.isDouble() || value.isNull()) {
    // these should be safe
  } else if (value.isObject()) {
    Variant val = sxe_get_value(value.toObject());
    // TODO: Assign it to value
  } else {
...
Variant f_simplexml_load_string(const String& data,
  ...
  xmlDocPtr document = xmlReadMemory(data.data(), data.size(), nullptr,
                                     nullptr, options);
  if (!document) {
    return false;
  }
...
void c_SimpleXMLElement::t___construct(const String& data,
                                       ...
                                       const String& ns /* = "" */,
                                       bool is_prefix /* = false */) {
  xmlDocPtr docp = data_is_url ? xmlReadFile(data.data(), nullptr, options) :
...
Variant c_SimpleXMLElement::t_xpath(const String& path) {
  ...
  xmlNsPtr* ns = xmlGetNsList(document, nodeptr);
  int64_t nsnbr = 0;
  if (ns != nullptr) {
...
Variant c_SimpleXMLElement::t_asxml(const String& filename /* = "" */) {
  ...
  if (filename.size()) {
    ...
    if (node) {
      if (node->parent && (XML_DOCUMENT_NODE == node->parent->type)) {
        ...
        xmlNodeDumpOutput(outbuf, document, node, 0, 0, nullptr);
  ...
  if (node) {
    if (node->parent && (XML_DOCUMENT_NODE == node->parent->type)) {
      xmlChar* strval;
      int strval_len;
      xmlDocDumpMemoryEnc(document, &strval, &strval_len,
                          (const char*)document->encoding);
      ...
      xmlNodeDumpOutput(outbuf, document, node, 0, 0,
                        (const char*)document->encoding);
...
Array c_SimpleXMLElement::t_getdocnamespaces(bool recursive /* = false */) {
  // TODO: Support 2nd param "from_root".
  bool from_root = true;
  xmlNodePtr node = from_root ? xmlDocGetRootElement(document) : node;
  Array ret = Array::Create();
...
Variant c_SimpleXMLElement::t___get(Variant name) {
  return sxe_prop_dim_read(this, name, true, false, /* type */ 0); // TODO: type? 
}
...
Variant c_SimpleXMLElement::t_offsetget(CVarRef index) {
  return sxe_prop_dim_read(this, index, false, true, /* type */ 0); // TODO: type? 
}
...
Variant c_SimpleXMLElementIterator::t_key() {
  ...
  xmlNodePtr curnode = curobj.isNull() ?
                         nullptr : element_from_object(curobj)->node;
  if (curnode) {
